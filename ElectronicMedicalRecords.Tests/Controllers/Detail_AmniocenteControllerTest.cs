...
{
    [TestClass]
    public class Detail_AmniocenteControllerTest
    {
        CP24Team08Entities db = new CP24Team08Entities();
        Detail_AmniocenteController controller = new Detail_AmniocenteController();
        [TestMethod]
        public void CreateP()
        {
            var amniocenteList = db.Amniocentes.ToList();
            amniocenteList[1].ChiDinh = true;
            ...
            {
                var result = controller.Create(detail_Amniocente, amniocenteList, info.ID, multiplesModel) as Task<ActionResult>;
        ...
        {
            var info = db.InformationExaminations.AsNoTracking().First();
            var amniocenteList = db.Detail_Amniocente.Where(p => p.InformationExamination_ID == info.ID).ToList();
            MultiplesModel multiplesModel = new MultiplesModel();
            multiplesModel.Detail_Amniocentes = amniocenteList;
            using (var scope = new TransactionScope())
            {
