...
{
    [TestClass]
    public class Detail_UrineControllerTest
    {
        CP24Team08Entities db = new CP24Team08Entities();
        Detail_UrineController controller = new Detail_UrineController();
        [TestMethod]
        public void CreateP()
        {
        ...
        {
            var info = db.InformationExaminations.AsNoTracking().First();
            var UrineList = db.Detail_Urine.Where(p => p.InfomationExamination_ID == info.ID).ToList();
            MultiplesModel multiplesModel = new MultiplesModel();
            multiplesModel.Detail_Urines = UrineList;
            using (var scope = new TransactionScope())
            {
