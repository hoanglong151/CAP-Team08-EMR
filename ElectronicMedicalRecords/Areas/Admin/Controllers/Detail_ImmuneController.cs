...
namespace ElectronicMedicalRecords.Areas.Admin.Controllers
{
    ...
    public class Detail_ImmuneController : Controller
    {
        ...
        {
            List<Detail_Immune> detail_Immunes = db.Detail_Immune.Where(p => p.InformationExamination_ID == multiplesModel.InformationExamination.ID).AsNoTracking().ToList();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateOldPatient(Detail_Immune detail_Immune, List<Immune> iMmunes, int informationID, MultiplesModel multiplesModel)
        {
            foreach (var item in iMmunes)
            {
                var checkexist = db.Detail_Immune.Where(p => p.InformationExamination_ID == informationID).FirstOrDefault(c => c.ID == item.ID);
